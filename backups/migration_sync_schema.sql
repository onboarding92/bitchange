-- Migration to sync database with TypeScript schema
-- Created: 2024-12-19
...
-- Step 3: Add missing referral columns
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `referralCode` VARCHAR(20) UNIQUE AFTER `ipWhitelist`;
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `referredBy` INT AFTER `referralCode`;

-- Step 4: Update antiPhishingCode column length if needed
ALTER TABLE `users` MODIFY COLUMN `antiPhishingCode` VARCHAR(50);

-- Step 5: Update twoFactorSecret column length if needed

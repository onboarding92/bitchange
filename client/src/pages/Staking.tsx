              ...
              <Card key={plan.id} className="glass border-primary/20 hover:border-primary/40 transition-all">
                <CardHeader>
                  ...
                  <CardDescription className="text-2xl font-bold text-primary">
                    {safeToFixed(parseFloat(plan.apr), 2)}% APR
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{safeToFixed(parseFloat(plan.minAmount), 2)} {plan.asset}</span>
                    </div>
                  </div>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4 py-4">
                        ...
                        <div className="text-sm text-muted-foreground space-y-1">
                          <div>APR: {safeToFixed(parseFloat(plan.apr), 2)}%</div>
                          <div>Lock Period: {plan.lockDays} days</div>
                          <div>
                            Estimated Reward: {stakeAmount ? safeToFixed((parseFloat(stakeAmount) * parseFloat(plan.apr) * plan.lockDays) / (365 * 100), 4) : "0"} {plan.asset}
                          </div>
                        </div>
                        <Button
                          className="w-full"
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{safeToFixed(parseFloat(position.amount), 4)} {plan?.asset}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Current Reward:</span>
                          <span className="font-medium text-green-500">+{safeToFixed(reward, 4)} {plan?.asset}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">APR:</span>
                          <span className="font-medium">{plan ? safeToFixed(parseFloat(plan.apr), 2) : "0"}%</span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    {completedPositions.map((position) => {
                      ...
                      return (
                        <tr key={position.id} className="border-b border-border/50">
                          ...
                          <td className="text-right py-3 px-4">{safeToFixed(parseFloat(position.amount), 4)} {plan?.asset}</td>
                          <td className="text-right py-3 px-4 text-green-500">+{safeToFixed(parseFloat(position.rewards), 4)} {plan?.asset}</td>
